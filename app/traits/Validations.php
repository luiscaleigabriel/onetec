...
trait Validations
{
    public function unique($field, $param)
    {
        ...
        if ($registerFound) {
            Flash::set($field, "O email {$data} já está registrado");
            return null;
        }
    ...
    public function email($field)
    {
        if (!filter_input(INPUT_POST, $field, FILTER_VALIDATE_EMAIL)) {
            Flash::set($field, "Informe um email válido");
            return null;
        }
    ...
    public function maxLen($field, $param)
    {
        ...
        if (strlen($data) > $param) {
            Flash::set($field, "Esse campo tem de ter no máximo {$param} caracteres");
            return null;
        }
